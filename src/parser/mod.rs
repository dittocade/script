...
use winnow::{
    combinator::{alt, delimited, opt, repeat, separated, separated_foldl1, separated_foldr1, seq, terminated}, error::StrContext, token::one_of, Parser, Result
};
...
pub fn boolean(i: &mut Tokens) -> Result<bool> {
    Kind::Boolean.map(|&v| v.value).map(|v| match v {
        "True" => true,
        "False" => false,
        _ => unreachable!()
    }).parse_next(i)
...
pub fn modifier(i: &mut Tokens) -> Result<Modifier> {
    Kind::Modifier.map(|v| match v.value {
        "$" => Modifier::Global,
        "!" => Modifier::Saved,
        _ => unreachable!()
    }).parse_next(i)
...
pub fn prefix_expression(i: &mut Tokens) -> Result<Expression> {
    (
        repeat(.., one_of([Kind::Operator(Operator::Not), Kind::Operator(Operator::Subtract)])),
        simple_expression
        ...
        ops.iter().fold(a, |a, op| {
            ...
            Expression::Call {
                ...
                inputs: vec![Input { label: None, value: a }],
            }
        })
    }).parse_next(i)
}
...
pub fn exponentative_expression(i: &mut Tokens) -> Result<Expression> {
    separated_foldr1(
        ...
        Expression::Call {
            ...
            inputs: vec![Input { label: None, value: a }, Input { label: None, value: b }],
        }
    }).parse_next(i)
}
...
pub fn multiplicative_expression(i: &mut Tokens) -> Result<Expression> {
    separated_foldl1(
        ...
        one_of([Kind::Operator(Operator::Multiply), Kind::Operator(Operator::Divide)]),
        |a, op, b| {
        let Kind::Operator(op) = op.kind else {
            unreachable!();
        };
...
pub fn additive_expression(i: &mut Tokens) -> Result<Expression> {
    separated_foldl1(
        ...
        one_of([Kind::Operator(Operator::Add), Kind::Operator(Operator::Subtract)]),
        |a, op, b| {
        let Kind::Operator(op) = op.kind else {
            unreachable!();
        };
...
pub fn relational_expression(i: &mut Tokens) -> Result<Expression> {
    separated_foldl1(
        additive_expression,
        one_of([Kind::Operator(Operator::LessThan), Kind::Operator(Operator::AtMost), Kind::Operator(Operator::GreaterThan), Kind::Operator(Operator::AtLeast)]),
        |a, op, b| {
        let Kind::Operator(op) = op.kind else {
            unreachable!();
        };
...
pub fn equality_expression(i: &mut Tokens) -> Result<Expression> {
    separated_foldl1(
        ...
        one_of([Kind::Operator(Operator::Equal), Kind::Operator(Operator::NotEqual)]),
        |a, op, b| {
        let Kind::Operator(op) = op.kind else {
            unreachable!();
        };
...
pub fn comment(i: &mut Tokens) -> Result<Statement> {
    Kind::Comment.map(|v| Statement::Comment(v.value[1..].trim().to_string())).parse_next(i)
}
